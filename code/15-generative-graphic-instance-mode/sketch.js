...
new p5(function(p) {
  ...
  p.setup = function() {
	...
	var centerRightY = 0.2*(globals.padYmax-globals.padYmin)+globals.padYmin;
  	...
  	for(var i = globals.padXmin; i <= globals.padXmax; i+=Math.round(globals.padXmin)){
  		for(var j = globals.padYmin; j <= globals.padYmax; j+=Math.round(globals.padYmin)){
	    	...
	    	p.noStroke();
	  	 	p.fill(200, 150, 50, 5);
	  	 	p.ellipse(centerRightX, centerRightY, globals.padXmin, globals.padYmin);
	  	 	
	  	 	// draw rays shift to x center-right and y center
	  	 	//p.line(centerRightX, centerRightY, 0.5*(globals.padXmax-globals.padXmin)+globals.padXmin+0.5*(i-globals.padXmin), j);
	  	 	p.strokeWeight(globals.padXmin*0.02);
	  	 	p.stroke(200, 150, 50, 40);
	  	 	p.line(centerRightX, centerRightY, i, j);
	        ...
	        p.push();
	      	...
	      	p.tanRotate = p.atan2(centerRightY-j, centerRightX-i);
